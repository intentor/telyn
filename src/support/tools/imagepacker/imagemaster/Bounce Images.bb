...
testimages=LoadPack("..\packs\testpack.png")
numImages=CountPackedImages(testimages)

; grab all images from the pack
Dim img.ImageType(numImages)
For n=1 To numImages
	img(n)=New imageType
	img(n)\image=GetPackedImage(n,testimages)
	img(n)\numframes=PackedImageFrames(n,testimages)
